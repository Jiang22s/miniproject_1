        ...
        JavaPairRDD<Integer, Double> newRanks =
                sites.
                        ...
                        flatMapToPair(kv-> {
                            ...
                            Website links = values._1();
                            Double currentRanks = values._2();

                            List<Tuple2<Integer, Double>> contribs =
                                    new LinkedList<Tuple2<Integer, Double>>();
                            Iterator<Integer> iter = links.edgeIterator();
                            while(iter.hasNext()){
                                final int destID = iter.next();
                                contribs.add(new Tuple2(destID,currentRanks/ (double)links.getNEdges()));
                            }
                            return contribs;
